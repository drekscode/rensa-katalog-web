...
<div x-data="{ 
    ...
    <div class="sm:flex sm:items-center justify-between gap-4">
        <div class="sm:flex-auto">
            <p class="mt-2 text-sm text-gray-600">A list of all artikel in your catalog.</p>
        </div>

        <!-- Search Bar -->
        <div class="mt-4 sm:mt-0 flex-1 max-w-md">
            <form action="{{ route('admin.artikel.index') }}" method="GET" class="relative" id="searchFormArtikel">
                ...
                <input type="text" 
                       ...
                       @input="document.getElementById('searchFormArtikel').submit()"
                       ...
                       placeholder="Search artikel by title or category...">
            </form>
        </div>
                        ...
                        <tbody class="divide-y divide-gray-200 bg-white">
                            ...
                            <tr class="hover:bg-gray-50/50 transition-colors">
                                ...
                                <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                    @if($artikel->foto)
                                        <img src="{{ asset('storage/' . $artikel->foto) }}" 
                                             ...
                                             @click="$dispatch('open-lightbox', { url: '{{ asset('storage/' . $artikel->foto) }}' })">
                                    @else
                                ...
                                <td class="px-3 py-4 text-sm text-gray-600">
                                    <span class="block font-medium text-gray-900">{{ $artikel->hastag_kategori }}</span>
                                    <span class="block text-gray-500 truncate max-w-xs">{{ Str::limit($artikel->deskripsi, 50) }}</span>
                                </td>
                                <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-600">
                                ...
                                <td class="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                                    <div class="flex items-center justify-end gap-2">
                                        <button @click="openViewModal({
                                            ...
                                            'foto': '{{ $artikel->foto ? asset('storage/' . $artikel->foto) : '' }}'
                                        })"
                                        ...
                                        <form action="{{ route('admin.artikel.destroy', $artikel) }}" 
                                            ...
                                            <button type="button" 
                                                    @click="$dispatch('confirm-delete', { 
                                                        ...
                                                        message: 'Are you sure you want to delete \'{{ addslashes($artikel->judul) }}\'? This will permanently remove it from the blog.',
                            ...
                            <tr>
                                <td colspan="7" class="px-3 py-12 text-center text-sm text-gray-500">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="text-gray-900 font-medium mb-1">No artikel found</p>
                                        <p class="text-gray-500 mb-4">Get started by creating a new artikel.</p>
                                        <a href="{{ route('admin.artikel.create') }}" class="text-[#8b9b7e] hover:text-[#7a8a6f] font-medium">Create artikel →</a>
                                    </div>
                                </td>
                            </tr>
                            @endforelse
